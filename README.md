...
1. import to metamask the account created previously using geth:
     - use `% geth —rinkeby acount list`  to find where the keystore file is stored. From the
        import menu, select `json file` option as opposed to `Private key`. In the
        `Enter password` type the Passprase (for example `test`) used when you created 
        (eg with geth) the account that you are importing.
